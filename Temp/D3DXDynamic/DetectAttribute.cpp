...
CDetectAttribute::CDetectAttribute()
{
	//Set view matrix.
	viewMatrix.identity();
	//Rotate matrix.
...
CDetectAttribute::~CDetectAttribute(void)
{
	//Disable detection.
	disableDetection();
	//Clear value.
...
_BOOL CDetectAttribute::isDetecting()
{
	//Value.
	_BOOL detectFace;
	//Open lock.
...
C4DQuaternion CDetectAttribute::getBallRotation() const
{
	//Rotation.
	return C4DQuaternion(*controlBall.GetQuatNow());
}
...
void CDetectAttribute::getFaceCenter(_INTEGER& x,_INTEGER& y) const
{
	//Set coordination.
...
void CDetectAttribute::disableDetection()
{
	//Open lock.
	openLock();
	{
...
_BOOL CDetectAttribute::doDetection(_OBJECT captureImage)
{
#ifdef _DEBUG
	assert(captureImage != _NULL);
#endif
#ifdef _DEBUG
	...
	{
		//Convert image.
		cvConvertImage((IplImage *)captureImage,(IplImage *)convertedImage);
		...
		for(_INTEGER i = 0;i < faces->total;i ++)
		{
			...
			//Draw rectangle.
			cvRectangle((IplImage *)captureImage,pt1,pt2,CV_RGB(255,0,0),3);
		}
		//Check max area.
...
void CDetectAttribute::rotateViewPort(CViewPort* viewPort)
{
#ifdef _DEBUG
...
_BOOL CDetectAttribute::enableDetection(_INTEGER width,_INTEGER height)
{
#ifdef _DEBUG

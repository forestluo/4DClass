...
CDXUTApplication::CDXUTApplication(void)
{
	...
	//检查Joint的绘制和操作情况。
	CSceneApplication::setForeground(new CJointScene());

	//检测多个随机几何体之间的绘制、操作和相互关系问题。
	//CSceneApplication::setForeground(new CRandomScene());

	//检测简单组合体的绘制和操作问题。
	//CSceneApplication::setForeground(new CCompoundScene());

	//用于检测模拟设备状态下的绘制问题。
	//当三角片的数量在一定范围内的时候，系统运行正常。
	//当三角片的数量超过一定数值的时候，系统会出现异常。
	//CSceneApplication::setForeground(new CSphere0Scene());

	//用于检测最简单的几何体绘制和操作问题。
	//CSceneApplication::setForeground(new CTriangle0Scene());

	//用于检测所有基础集合体的绘制和操作问题。
	//CSceneApplication::setForeground(new CPrimitiveScene());
}
...
HRESULT CDXUTApplication::handleMessage(const D3DXEvent& d3dxEvent)
{
	...
	case d3dxMsgProc:
		...
		{
			//Release controller.
			if(CPhysicsApplication::isRunning()) CPhysicsApplication::clearController(); 
		}

...
HRESULT DXUtil_GetDXSDKMediaPathCch( TCHAR* strDest, int cchDest )
{
    ...
    if( lstrlen(strDest) + lstrlen(strMedia) < cchDest )
        _tcscat_s( strDest, MAX_PATH, strMedia );
    else
        return E_INVALIDARG;
...
HRESULT DXUtil_FindMediaFileCch( TCHAR* strDestPath, int cchDest, TCHAR* strFilename )
{
    ...
    if( INVALID_HANDLE_VALUE != file )
    {
        _tcsncpy_s( strDestPath, MAX_PATH, strShortName, cchDest );
    ...
    _tcsncpy_s( strDestPath, MAX_PATH, strFilename, cchDest );
    strDestPath[cchDest-1] = 0; // _tcsncpy doesn't NULL term if it runs out of space
...
#ifdef _UNICODE
    wcsncpy_s( wstrDestination, MAX_PATH, tstrSource, cchDestChar );
...
#ifdef _UNICODE
    wcsncpy_s( tstrDestination, MAX_PATH, wstrSource, cchDestChar );
...
VOID DXUtil_LaunchReadme( HWND hWnd, TCHAR* strLoc )
{
    ...
    if( !bFound )
    {
        ...
        _tcscpy_s( strReadmePath, MAX_PATH, strExePath );
...
{
    int nResult = _sntprintf_s( strDest, MAX_PATH, cchDestChar, TEXT("{%0.8X-%0.4X-%0.4X-%0.2X%0.2X-%0.2X%0.2X%0.2X%0.2X%0.2X%0.2X}"),

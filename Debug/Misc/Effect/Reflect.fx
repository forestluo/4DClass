...
texture g_txScene : SCENEMAP // texture for scene rendering
<
    string SasUiLabel = "Scene Map";
    string SasUiControl= "FilePicker";
>;

texture g_txEnvMap : ENVIRONMENTMAP // texture for environment map
<
...
shared float4x4 g_mProj : PROJECTION
<
    bool SasUiVisible = false;
    string SasBindAddress= "Sas.Camera.Projection";
>;   

shared float4 g_vLightColor : LIGHTCOLOR
<  
                 ...
                 float3 EnvTex : TEXCOORD3 ) : COLOR0
{
    ...
    float4 fSpecular = pow( abs(saturate( dot( vHalf, normalize( ViewNormal ) ) ) * Specular), Power );

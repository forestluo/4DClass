...
typedef VOID (CALLBACK *PROGRESSCALLBACK)(LPWSTR , DWORD);
...
typedef struct
{
	WCHAR	psName[256];				  //name of driver
...
public:
	...
	//
	BOOL	ConnectWithDriver(LPCWSTR Name=NULL);//connect by name
	...
	BOOL	SavePalette(LPCWSTR file);
	BOOL	LoadPalette(LPCWSTR file);
	BOOL	PastePalette();
	...
	LPCWSTR	GetDriverName(); //get current driver name
	LPCWSTR	GetDriverVer();	 //get current driver version string
...
protected:
	...
	void	_notify(LPWSTR lp=NULL);
	void	_notify(LPWSTR lp, DWORD pc);
	void	_selfInit();
	void	_getDrvList();
	BOOL	_getCaps();
	BOOL	_getStatus();
...
inline	void	CAviCap::_notify(LPWSTR lp)
	{if(_progressCB) (*_progressCB)(lp,100);}
inline	void	CAviCap::_notify(LPWSTR lp, DWORD pc)
	{if(_progressCB) (*_progressCB)(lp,pc);}
inline	BOOL	CAviCap::SetFrameSize(CSize sz)
	{return SetFrameSize(sz.cx,sz.cy);}
inline	LPCWSTR	CAviCap::GetDriverName()	
	{return (_curDriver==-1||!_drvNames.GetSize()) ? NULL : (LPCWSTR)_drvNames[_curDriver];}
inline	LPCWSTR	CAviCap::GetDriverVer()	
	{return (_curDriver==-1||!_drvNames.GetSize()) ? NULL : (LPCWSTR)_drvVers[_curDriver];}
inline	int		CAviCap::GetDriverIndex() 
	{return _curDriver;}
inline	BOOL	CAviCap::SavePalette(LPCWSTR file) 
	{return (_curDriver==-1) ? FALSE : (BOOL)capPaletteSave(GetSafeHwnd(), file);}
inline	BOOL	CAviCap::LoadPalette(LPCWSTR file) 

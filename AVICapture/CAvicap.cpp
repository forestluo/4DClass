...
LPWSTR	lpNotify01	=	L"Testing resolutions...";
LPWSTR	lpNotify02	=	L"Seaching for NTSC formats...";
LPWSTR	lpNotify020 =	L"Seaching for PAL/SECAM formats...";
LPWSTR	lpNotify03	=	L"Seaching for buffers...";
...
BOOL	CAviCap::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd,  BOOL fAutoSize)
{
	...
	HWND hWnd=capCreateCaptureWindow(L"AviCap_Basic", 
...
//
BOOL	CAviCap::ConnectWithDriver(LPCWSTR Name)
{
...
BOOL	CAviCap::SetErrorCallBack(LPVOID cb)
{
	CHECKWIN();
	CHECKCNCT();
...
// Setup user callback for Image data access
// 
BOOL	CAviCap::SetFrameCallBack(FRAMECALLBACK cb)
{
	CHECKWIN();
	CHECKCNCT();
	_UserFrameCallBack=cb;
	return TRUE;	
}
...
//
// Setup required capture parameters.
// Do it at once!
BOOL	CAviCap::SetCapSetup(CAPTUREPARMS *parms)
{
	CHECKWIN();
	CHECKCNCT();
...
//
BOOL	CAviCap::SetFormat(LPBITMAPINFO bmpinf)
{
	CHECKWIN();
	CHECKCNCT();
...
// Testing
// Warning! It may be slow enough!
BOOL	CAviCap::EnumValidFrameSize(CSize &sz, int indx)
{
	CHECKWIN();
	CHECKCNCT();
...
// One more testing...
// Warning! It may be slow enough!
BOOL	CAviCap::IsSupportedBitsResolution(UINT bitsCount)
{
	CHECKWIN();
	CHECKCNCT();
...
// Standard AVIcap window message 
// "capDriverGetCaps" macro wrapper
BOOL	CAviCap::GetDriverCaps(CAPDRIVERCAPS *caps)
{
	CHECKWIN();
	CHECKCNCT();
...
// Standard AVIcap window message 
// "capCaptureGetSetup" macro wrapper
BOOL	CAviCap::GetCapSetup(CAPTUREPARMS *parms)
{
	CHECKWIN();
	CHECKCNCT();
	if(!parms)	return FALSE;
...
// 
// no comments
BOOL	CAviCap::GetDriversList(CStringArray *Names, CStringArray *Vers)
{
	if(!_totalDrv())
		_getDrvList();

	if(Names)
		Names->Copy(_drvNames);
	if(Vers)
		Vers->Copy(_drvVers);
	return 	 _totalDrv();
}
...
//
int	CAviCap::_GetInternalBuffersCount()
{
	CHECKWIN();
	CHECKCNCT();
...
void	CAviCap::_getDrvList()
{
    ...
    for (j =0; j < MAXCAPDRIVERS; j++) 
        if (capGetDriverDescription (j,
                ...
                szVer,  sizeof (szVer))) 
	{
           _drvNames.Add((LPCWSTR)szName);
	   _drvVers.Add((LPCWSTR)szVer);
        }
    	else
	{
	break; // !!!! terminate 
...
#define checkQuit()	{if(_capAbort) {capDriverDisconnect(GetSafeHwnd()); _capAbort=FALSE; return FALSE;}}
...
// See notes for _testFrameFormat
// ++Note: Testing only standard sizes(see below) and color resolution
...
LPBITMAPINFO	CAviCap::_mk_LPBITMAPINFO(UINT biBitCount,int x, int y)
{
	LPBITMAPINFO bmi;
	int sz;
...
int	CAviCap::_getBMIsize(LPBITMAPINFO bmi)
{
	LPBITMAPINFOHEADER bH=(LPBITMAPINFOHEADER)bmi;
...
DWORD	CAviCap::_calcBufferSize(LPBITMAPINFOHEADER bih)
{
...
int	CAviCap::_getBMIsize(UINT bitsReq, UINT bmiColors)
{
	int sz=0;
	...
	switch(bitsReq)
		...
		case 16:
		case 32:
		default: break;	/*  16,24,32-bit DIB has no color table */
		}
...
void CAviCap::OnTimer(UINT nIDEvent) 
{
...
BOOL CAviCap::OnEraseBkgnd(CDC* pDC) 
{
...
void CAviCap::OnLButtonDown(UINT nFlags, CPoint point) 
{
	//add additional processing here
}

void CAviCap::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	//add additional processing here
}

void CAviCap::OnPaint(void)
{
	//note AVICAP window doesn't performs a stretching,
	//so we have to do this by themself.
	LPBITMAPINFOHEADER bmpIH=( LPBITMAPINFOHEADER )_bmpInfo;

	if(_fShow && _curDriver!=-1)//switch off/on
		{
		ASSERT(_bmpInfo);

...
int CClassView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	// Load images:
	m_mainToolbar.Create(this, AFX_DEFAULT_TOOLBAR_STYLE, IDR_SORT);
	m_mainToolbar.LoadToolBar(IDR_SORT, 0, 0, TRUE /* Is locked */);

	OnChangeVisualStyle();

	m_mainToolbar.SetPaneStyle(m_mainToolbar.GetPaneStyle() | CBRS_TOOLTIPS | CBRS_FLYBY);
	m_mainToolbar.SetPaneStyle(m_mainToolbar.GetPaneStyle() & ~(CBRS_GRIPPER | CBRS_SIZE_DYNAMIC | CBRS_BORDER_TOP | CBRS_BORDER_BOTTOM | CBRS_BORDER_LEFT | CBRS_BORDER_RIGHT));

	m_mainToolbar.SetOwner(this);

	// All commands will be routed via this control , not via the parent frame:
	m_mainToolbar.SetRouteCommandsViaFrame(FALSE);

	CMenu menuSort;
	menuSort.LoadMenu(IDR_POPUP_SORT);

	m_mainToolbar.ReplaceButton(ID_SORT_MENU, CClassViewMenuButton(menuSort.GetSubMenu(0)->GetSafeHmenu()));

	CClassViewMenuButton* pButton =  DYNAMIC_DOWNCAST(CClassViewMenuButton, m_mainToolbar.GetButton(0));
...
void CClassView::FillClassView()
{
	...
	m_wndClassView.InsertItem(_T("m_mainToolbar"), 6, 6, hClass);
...
void CClassView::AdjustLayout()
{
	...
	int cyTlb = m_mainToolbar.CalcFixedLayout(FALSE, TRUE).cy;

	m_mainToolbar.SetWindowPos(NULL, rectClient.left, rectClient.top, rectClient.Width(), cyTlb, SWP_NOACTIVATE | SWP_NOZORDER);
...
/*
	...
	m_mainToolbar.CleanUpLockedImages();
	m_mainToolbar.LoadBitmap(theApp.m_bHiColorIcons ? IDB_SORT_24 : IDR_SORT, 0, 0, TRUE);
*/
}

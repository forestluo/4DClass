...
void DemoEntityManager::LoadScene (const char* const fileName)
{
	...
	for (dList<NewtonBody*>::dListNode* bodyNode = bodyList.GetFirst(); bodyNode; bodyNode = bodyNode->GetNext()) {
		...
		NewtonBody* body = bodyNode->GetInfo();
		dScene::dTreeNode* sceneNode = (dScene::dTreeNode*)NewtonBodyGetUserData(body);
		DemoEntity* entity = entDictionary.Find(sceneNode)->GetInfo();
		NewtonBodySetUserData(body, entity);

		// see if this body have some special setups
		dScene::dTreeNode* node = database.FindChildByType(sceneNode, dRigidbodyNodeInfo::GetRttiType());
		_ASSERTE (node);
		dRigidbodyNodeInfo* bodyData = (dRigidbodyNodeInfo*) database.GetInfoFromNode(node);

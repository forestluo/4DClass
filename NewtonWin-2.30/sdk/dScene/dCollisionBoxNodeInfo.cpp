...
bool dCollisionBoxNodeInfo::Deserialize (TiXmlElement* rootNode, int revisionNumber) 
{
...
void dCollisionBoxNodeInfo::SerializeBinary (FILE* file) 
{
_ASSERTE (0);
...
void dCollisionBoxNodeInfo::DrawWireFrame(dScene* const world, dScene::dTreeNode* const myNode, const dVector& color) const
{
...
void dCollisionBoxNodeInfo::DrawSolidWireFrame(dScene* const world, dScene::dTreeNode* const myNode, const dVector& color) const
{
	_ASSERTE (world->GetInfoFromNode(myNode) == this);
	_ASSERTE (myNode == world->Find(GetUniqueID()));
_ASSERTE (0);
...
void dCollisionBoxNodeInfo::DrawGouraudShaded(dScene* const world, dScene::dTreeNode* const myNode, const dVector& color) const
{
	_ASSERTE (world->GetInfoFromNode(myNode) == this);
	_ASSERTE (myNode == world->Find(GetUniqueID()));
...
dCollisionBoxNodeInfo::dGizmoHandle dCollisionBoxNodeInfo::GetHighlightedGizmoHandle(
	dScene* world, 
...
void dCollisionBoxNodeInfo::DrawGizmoHandle(dScene* world, const dMatrix& coordinaSystem, dGizmoMode mode, dGizmoHandle handle, const dVector& color, dFloat size) const
{
_ASSERTE (0);
...
void dCollisionBoxNodeInfo::DrawGizmo(
	dScene* world, 
	dScene::dTreeNode* myNode, 
	const dMatrix& coordinaSystem,
	const dVector& color, 

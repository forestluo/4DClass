...
dCollisionCapsuleNodeInfo::dCollisionCapsuleNodeInfo(NewtonCollision* cylinder)
	:dCollisionNodeInfo () 
{
	NewtonCollisionInfoRecord record;
	NewtonCollisionGetInfo(cylinder, &record);
	_ASSERTE (record.m_collisionType == SERIALIZE_ID_CAPSULE);
...
void dCollisionCapsuleNodeInfo::CalculateInertiaGeometry (dScene* world, dVector& inertia, dVector& centerOfMass) const
{
...
void dCollisionCapsuleNodeInfo::Serialize (TiXmlElement* rootNode) const
{
...
bool dCollisionCapsuleNodeInfo::Deserialize (TiXmlElement* rootNode, int revisionNumber) 
{
...
void dCollisionCapsuleNodeInfo::SerializeBinary (FILE* file) 
{
_ASSERTE (0);
...
void dCollisionCapsuleNodeInfo::DrawWireFrame(dScene* const world, dScene::dTreeNode* const myNode, const dVector& color) const
{
...
void dCollisionCapsuleNodeInfo::DrawSolidWireFrame(dScene* const world, dScene::dTreeNode* const myNode, const dVector& color) const
{
	_ASSERTE (world->GetInfoFromNode(myNode) == this);
	_ASSERTE (myNode == world->Find(GetUniqueID()));
_ASSERTE (0);
...
void dCollisionCapsuleNodeInfo::DrawGouraudShaded(dScene* const world, dScene::dTreeNode* const myNode, const dVector& color) const
{
	_ASSERTE (world->GetInfoFromNode(myNode) == this);
	_ASSERTE (myNode == world->Find(GetUniqueID()));
...
dCollisionCapsuleNodeInfo::dGizmoHandle dCollisionCapsuleNodeInfo::GetHighlightedGizmoHandle(
	dScene* world, 
...
void dCollisionCapsuleNodeInfo::DrawGizmoHandle(dScene* world, const dMatrix& coordinaSystem, dGizmoMode mode, dGizmoHandle handle, const dVector& color, dFloat size) const
{
_ASSERTE (0);
...
void dCollisionCapsuleNodeInfo::DrawGizmo(
	dScene* world, 
	dScene::dTreeNode* myNode, 
	const dMatrix& coordinaSystem,
	const dVector& color, 
...
{
	_ASSERTE (IsType (dCollisionCapsuleNodeInfo::GetRttiType()));

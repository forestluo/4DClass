...
dCollisionSphereNodeInfo::dCollisionSphereNodeInfo(NewtonCollision* sphere)
	:dCollisionNodeInfo () 
{
	NewtonCollisionInfoRecord record;
	NewtonCollisionGetInfo(sphere, &record);
	_ASSERTE (record.m_collisionType == SERIALIZE_ID_SPHERE);

	dMatrix& offsetMatrix = *((dMatrix*) record.m_offsetMatrix);
	SetName ("sphere collision");
	m_radius = dVector (record.m_sphere.m_r0, record.m_sphere.m_r1, record.m_sphere.m_r2); 
...
void dCollisionSphereNodeInfo::SetRadius (const dVector& radius)
{
	m_radius = radius;
	m_radius.m_w = 0.0f;
}

const dVector& dCollisionSphereNodeInfo::GetRadius () const
{
	return m_radius; 
}
...
void dCollisionSphereNodeInfo::BakeTransform (const dMatrix& transform)
{
	...
	_ASSERTE (0);
//	m_radius = transform.RotateVector(m_radius);
}


void dCollisionSphereNodeInfo::CalculateInertiaGeometry (dScene* world, dVector& inertia, dVector& centerOfMass) const
{
	NewtonWorld* newton = world->GetNewtonWorld();
	NewtonCollision* box = NewtonCreateSphere(newton, m_radius.m_x, m_radius.m_y, m_radius.m_z, 0, &m_matrix[0][0]);
...
void dCollisionSphereNodeInfo::Serialize (TiXmlElement* rootNode) const
{
	SerialiseBase(dCollisionNodeInfo, rootNode);

	char tmp[1024];
	TiXmlElement* dataNode = new TiXmlElement ("radius");
	rootNode->LinkEndChild(dataNode);
	dFloatArrayToString (&m_radius[0], 4, tmp, sizeof (tmp));
	dataNode->SetAttribute("float4", tmp);
}

bool dCollisionSphereNodeInfo::Deserialize (TiXmlElement* rootNode, int revisionNumber) 
{
...
void dCollisionSphereNodeInfo::SerializeBinary (FILE* file) 
{
_ASSERTE (0);
...
void dCollisionSphereNodeInfo::DrawWireFrame(dScene* const world, dScene::dTreeNode* const myNode, const dVector& color) const
{
...
void dCollisionSphereNodeInfo::DrawSolidWireFrame(dScene* const world, dScene::dTreeNode* const myNode, const dVector& color) const
{
	_ASSERTE (world->GetInfoFromNode(myNode) == this);
	_ASSERTE (myNode == world->Find(GetUniqueID()));
_ASSERTE (0);
...
void dCollisionSphereNodeInfo::DrawGouraudShaded(dScene* const world, dScene::dTreeNode* const myNode, const dVector& color) const
{
	_ASSERTE (world->GetInfoFromNode(myNode) == this);
	_ASSERTE (myNode == world->Find(GetUniqueID()));
...
dCollisionSphereNodeInfo::dGizmoHandle dCollisionSphereNodeInfo::GetHighlightedGizmoHandle(
	dScene* world, 
...
void dCollisionSphereNodeInfo::DrawGizmoHandle(dScene* world, const dMatrix& coordinaSystem, dGizmoMode mode, dGizmoHandle handle, const dVector& color, dFloat size) const
{
_ASSERTE (0);
...
void dCollisionSphereNodeInfo::DrawGizmo(dScene* const world, dScene::dTreeNode* const myNode, const dMatrix& coordinaSystem, const dVector& color, dGizmoMode mode, dFloat size) const
{
_ASSERTE (0);
...
{
	_ASSERTE (IsType (dCollisionSphereNodeInfo::GetRttiType()));
	...
	return NewtonCreateSphere(world, m_radius.m_x, m_radius.m_y, m_radius.m_z, collisionID, &offsetMatrix[0][0]);

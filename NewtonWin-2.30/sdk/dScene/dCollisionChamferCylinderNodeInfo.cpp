...
dCollisionChamferCylinderNodeInfo::dCollisionChamferCylinderNodeInfo(dScene* world) 
...
dCollisionChamferCylinderNodeInfo::dCollisionChamferCylinderNodeInfo(NewtonCollision* cylinder)
	:dCollisionNodeInfo () 
{
	NewtonCollisionInfoRecord record;
	NewtonCollisionGetInfo(cylinder, &record);
	_ASSERTE (record.m_collisionType == SERIALIZE_ID_CHAMFERCYLINDER);
...
void dCollisionChamferCylinderNodeInfo::CalculateInertiaGeometry (dScene* world, dVector& inertia, dVector& centerOfMass) const
{
...
void dCollisionChamferCylinderNodeInfo::Serialize (TiXmlElement* rootNode) const
{
...
bool dCollisionChamferCylinderNodeInfo::Deserialize (TiXmlElement* rootNode, int revisionNumber) 
{
...
void dCollisionChamferCylinderNodeInfo::SerializeBinary (FILE* file) 
{
_ASSERTE (0);
...
void dCollisionChamferCylinderNodeInfo::DrawWireFrame(dScene* const world, dScene::dTreeNode* const myNode, const dVector& color) const
{
...
void dCollisionChamferCylinderNodeInfo::DrawSolidWireFrame(dScene* const world, dScene::dTreeNode* const myNode, const dVector& color) const
{
	_ASSERTE (world->GetInfoFromNode(myNode) == this);
	_ASSERTE (myNode == world->Find(GetUniqueID()));
_ASSERTE (0);
...
void dCollisionChamferCylinderNodeInfo::DrawGouraudShaded(dScene* const world, dScene::dTreeNode* const myNode, const dVector& color) const
{
	_ASSERTE (world->GetInfoFromNode(myNode) == this);
	_ASSERTE (myNode == world->Find(GetUniqueID()));
...
dCollisionChamferCylinderNodeInfo::dGizmoHandle dCollisionChamferCylinderNodeInfo::GetHighlightedGizmoHandle(
	dScene* world, 
...
void dCollisionChamferCylinderNodeInfo::DrawGizmoHandle(dScene* world, const dMatrix& coordinaSystem, dGizmoMode mode, dGizmoHandle handle, const dVector& color, dFloat size) const
{
_ASSERTE (0);
...
void dCollisionChamferCylinderNodeInfo::DrawGizmo(
	dScene* world, 
	dScene::dTreeNode* myNode, 
	const dMatrix& coordinaSystem,
	const dVector& color, 
...
{
	_ASSERTE (IsType (dCollisionChamferCylinderNodeInfo::GetRttiType()));

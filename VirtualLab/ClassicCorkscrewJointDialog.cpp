// ClassicCorkscrewJointDialog.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CClassicCorkscrewJointDialog, CDialogEx)

CClassicCorkscrewJointDialog::CClassicCorkscrewJointDialog(bool bChildOnly,CVirtualLabDoc* pDoc,CWnd* pParent /*=NULL*/)
	: CDialogEx(CClassicCorkscrewJointDialog::IDD, pParent)
...
CClassicCorkscrewJointDialog::~CClassicCorkscrewJointDialog()
{
	//Clear value.
	m_pDoc = NULL;
}

void CClassicCorkscrewJointDialog::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CClassicCorkscrewJointDialog, CDialogEx)
	ON_BN_CLICKED(IDOK, &CClassicCorkscrewJointDialog::OnBnClickedOk)
END_MESSAGE_MAP()


// CClassicCorkscrewJointDialog message handlers

BOOL CClassicCorkscrewJointDialog::OnInitDialog()
{
...
void CClassicCorkscrewJointDialog::OnBnClickedOk()
{
	//Update data.
	UpdateData(TRUE);
	...
	C4DVector3 direction = rotation.rotate(CWorldJointObject::getDefaultMajorDirection(CJointAttribute::classicCorkscrewJoint));
	//Set direction.
	attribute.jointData.jointClassicCorkscrew.direction[0] = direction.x();
	attribute.jointData.jointClassicCorkscrew.direction[1] = direction.y();
	attribute.jointData.jointClassicCorkscrew.direction[2] = direction.z();
	//Create joint.

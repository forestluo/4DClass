// CustomHingeJointDialog.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CCustomHingeJointDialog, CDialogEx)

CCustomHingeJointDialog::CCustomHingeJointDialog(CVirtualLabDoc* pDoc,CWnd* pParent /*=NULL*/)
	: CDialogEx(CCustomHingeJointDialog::IDD, pParent)
	, m_strChild(_T(""))
	, m_strParent(_T(""))
	...
	, m_nZRMajor(0)
{
	...
	CJointAttribute attribute = CWorldJointObject::getDefaultAttribute(CJointAttribute::customHingeJoint);
	//Set major.
	m_fXMajor = attribute.jointData.jointCustomHinge.direction[0];
	m_fYMajor = attribute.jointData.jointCustomHinge.direction[1];
	m_fZMajor = attribute.jointData.jointCustomHinge.direction[2];
}

CCustomHingeJointDialog::~CCustomHingeJointDialog()
{
	//Clear value.
	m_pDoc = NULL;
}

void CCustomHingeJointDialog::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CCustomHingeJointDialog, CDialogEx)
	ON_BN_CLICKED(IDOK, &CCustomHingeJointDialog::OnBnClickedOk)
END_MESSAGE_MAP()


// CCustomHingeJointDialog message handlers


BOOL CCustomHingeJointDialog::OnInitDialog()
{
...
void CCustomHingeJointDialog::OnBnClickedOk()
{
	//Update data.
	UpdateData(TRUE);
	...
	C4DVector3 direction = rotation.rotate(CWorldJointObject::getDefaultMajorDirection(CJointAttribute::customHingeJoint));
	//Set direction.
	attribute.jointData.jointCustomHinge.direction[0] = direction.x();
	attribute.jointData.jointCustomHinge.direction[1] = direction.y();
	attribute.jointData.jointCustomHinge.direction[2] = direction.z();
	//Create joint.

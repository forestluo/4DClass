                                         ...
                                         DWORD dwNumMaterials )
{
    ...
    if( d3dxMtrls && m_dwNumMaterials > 0 )
    {
        ...
        if( m_pMaterials == NULL )
            return E_OUTOFMEMORY;
		//Zero it.
		ZeroMemory(m_pMaterials,m_dwNumMaterials * sizeof(D3DMATERIAL9));

        m_pTextures = new LPDIRECT3DBASETEXTURE9[m_dwNumMaterials];
        if( m_pTextures == NULL )
            return E_OUTOFMEMORY;
		//Zero it.
		ZeroMemory(m_pTextures,m_dwNumMaterials * sizeof(LPDIRECT3DBASETEXTURE9));

		m_strMaterials = new CHAR[m_dwNumMaterials][MAX_PATH];
        if( m_strMaterials == NULL )
            return E_OUTOFMEMORY;
		//Zero it.
		ZeroMemory(m_strMaterials,m_dwNumMaterials * MAX_PATH * sizeof(CHAR));

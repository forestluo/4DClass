///////////////////////////////////////////////////////////////////////////////
//
// ConeAttribute.cpp
//  
// 4DClass Developer
// Copyright (c) 4DClass. All rights reserved.
//
///////////////////////////////////////////////////////////////////////////////

#include "ConeAttribute.h"

///////////////////////////////////////////////////////////////////////////////
//
// Including
//
///////////////////////////////////////////////////////////////////////////////

#include <memory.h>
#include "D3DX10Math.h"

#include "4DVector3.h"

///////////////////////////////////////////////////////////////////////////////
//
// Definition
//
///////////////////////////////////////////////////////////////////////////////

#define DEFAULT_RINGS										0
#define DEFAULT_SEGMENTS									16

///////////////////////////////////////////////////////////////////////////////
//
// Global
//
///////////////////////////////////////////////////////////////////////////////

//Cone0.
const CConeAttribute CConeAttribute::cone0(DEFAULT_SEGMENTS);
//Cone3.
const CConeAttribute CConeAttribute::cone3(3);
//Cone4.
const CConeAttribute CConeAttribute::cone4(4);
//Cone6.
const CConeAttribute CConeAttribute::cone6(6);

///////////////////////////////////////////////////////////////////////////////
//
// CConeAttribute
//
// Default construction.
//
/////////////////////////////////////////////////////////////////////////////////

CConeAttribute::CConeAttribute(void)
{
	//Create cone0.
	createAttribute(DEFAULT_RINGS,DEFAULT_SEGMENTS);
	//Set type and initialize default.
	type = CPrimitiveAttribute::cone0Primitive; initializeDefault();
}

///////////////////////////////////////////////////////////////////////////////
//
// CConeAttribute
//
// Default construction.
//
/////////////////////////////////////////////////////////////////////////////////

CConeAttribute::CConeAttribute(_INTEGER segments)
{
	//Create cone0.
	createAttribute(DEFAULT_RINGS,segments);
	//Check segments.
	switch(segments)
	{
	case DEFAULT_SEGMENTS:
		type = CPrimitiveAttribute::cone0Primitive;
		break;
	case 3:
		type = CPrimitiveAttribute::cone3Primitive;
		break;
	case 4:
		type = CPrimitiveAttribute::cone4Primitive;
		break;
	case 6:
		type = CPrimitiveAttribute::cone6Primitive;
		break;
	default:
#ifdef _DEBUG
		assert(_FALSE);
#endif
	}
	//Initialize default.
	initializeDefault();
}

///////////////////////////////////////////////////////////////////////////////
//
// CConeAttribute
//
// Default deconstruction.
//
/////////////////////////////////////////////////////////////////////////////////

CConeAttribute::~CConeAttribute(void)
{

}

/////////////////////////////////////////////////////////////////////////////////
//
// CreateAttribute
//
/////////////////////////////////////////////////////////////////////////////////

void CConeAttribute::createAttribute(_INTEGER rings,_INTEGER segments)
{
#ifdef _DEBUG
	assert(segments >= 3);
#endif
	//Set rings.
	this->rings = rings;
	//Set segments.
	this->segments = segments;

	//Index count.
	_INTEGER indexCount = 2 * segments;
	//Create index.
	CIndexAttribute::createShort3Index(indexCount);

	//Vertex count.
	_INTEGER vertexCount = 3 * segments + 2;
	//Create vertex.
	CVertexAttribute::createFloat3Vertex(vertexCount);

	//Normal count.
	_INTEGER normalCount = vertexCount;
	//Create normal.
	CNormalAttribute::createFloat3Normal(normalCount);

	//Texture count.
	_INTEGER textureCount = vertexCount;
	//Create texture.
	CTextureAttribute::createFloat2Texture(textureCount);

#ifdef _DEBUG
	assert(indexCount >= 6);
	assert(vertexCount >= 11);
	assert(vertexCount == normalCount);
	assert(vertexCount == textureCount);
#endif
}

///////////////////////////////////////////////////////////////////////////////
//
// GetPrimitiveAttribute
//
/////////////////////////////////////////////////////////////////////////////////

CPrimitiveAttribute CConeAttribute::getPrimitiveAttribute() const
{
	//Attribute.
	CPrimitiveAttribute attribute;
#ifdef _DEBUG
	assert(type == CPrimitiveAttribute::cone0Primitive ||
			type == CPrimitiveAttribute::cone3Primitive ||
			type == CPrimitiveAttribute::cone4Primitive ||
			type == CPrimitiveAttribute::cone6Primitive);
#endif
	//Set type.
	attribute.setType(type);
	//Copy data.
	attribute.rings = rings;
	attribute.segments = segments;
	//Copy data.
	attribute.primitiveData.cone.radius = radius;
	attribute.primitiveData.cone.height = height;
	//Return attribute.
	return attribute;
}

/////////////////////////////////////////////////////////////////////////////////
//
// InitializeDefault
//
/////////////////////////////////////////////////////////////////////////////////

void CConeAttribute::initializeDefault()
{
#ifdef _DEBUG
	assert(type == CPrimitiveAttribute::cone0Primitive ||
			type == CPrimitiveAttribute::cone3Primitive ||
			type == CPrimitiveAttribute::cone4Primitive ||
			type == CPrimitiveAttribute::cone6Primitive);
#endif
	//Set value.
	height = 1.0f;
	radius = 1.0f;
	//Setup attribute.
	setupAttribute();
	//Set type.
	this->type = type;
}

/////////////////////////////////////////////////////////////////////////////////
//
// Initialize
//
/////////////////////////////////////////////////////////////////////////////////

void CConeAttribute::initialize(_FLOAT radius,_FLOAT height)
{
#ifdef _DEBUG
	assert(type == CPrimitiveAttribute::cone0Primitive ||
			type == CPrimitiveAttribute::cone3Primitive ||
			type == CPrimitiveAttribute::cone4Primitive ||
			type == CPrimitiveAttribute::cone6Primitive);
#endif
	//Set radius.
	this->radius = radius;
	//Set height.
	this->height = height;
	//Setup attribute.
	setupAttribute();
}

///////////////////////////////////////////////////////////////////////////////
//
// SetPrimitiveAttribute
//
///////////////////////////////////////////////////////////////////////////////

void CConeAttribute::setPrimitiveAttribute(const CPrimitiveAttribute& attribute)
{
#ifdef _DEBUG
	assert(type == attribute.getType());
#endif
	//Initialize it.
	initialize(attribute.primitiveData.cone.radius,attribute.primitiveData.cone.height);
}

/////////////////////////////////////////////////////////////////////////////////
//
// SetupAttribute
//
/////////////////////////////////////////////////////////////////////////////////

void CConeAttribute::setupAttribute()
{
#ifdef _DEBUG
	assert(type == CPrimitiveAttribute::cone0Primitive ||
			type == CPrimitiveAttribute::cone3Primitive ||
			type == CPrimitiveAttribute::cone4Primitive ||
			type == CPrimitiveAttribute::cone6Primitive);
#endif

#ifdef _DEBUG
	assert(getIndexType() == CIndexAttribute::short3Index);
	assert(getVertexType() == CVertexAttribute::float3Vertex);
	assert(getNormalType() == CNormalAttribute::float3Normal);
	assert(getTextureType() == CTextureAttribute::float2Texture);
#endif

	//Get index.
	_SHORT3* index = CIndexAttribute::getShort3Index();
#ifdef _DEBUG
	assert(index != _NULL);
	assert(getIndexCount() >= 6);
#endif
	//Index array.
	_SHORT* indexArray = (_SHORT *)index;

	//Vertex index.
	_SHORT vertexIndex = 0;
	//For each segment, add a triangle to the sides triangle list.
	for(_INTEGER i = 0;i < segments;i ++)
	{
		//Set three indices (1 triangle) per segment.
		*indexArray = vertexIndex; indexArray ++; vertexIndex ++;
		*indexArray = vertexIndex; indexArray ++; vertexIndex += 2;
		//Check index.
		if(i == segments - 1)
		{
			*indexArray = 1; indexArray ++;	vertexIndex --;
		}
		else
		{
			*indexArray = vertexIndex; indexArray ++; vertexIndex --;	
		}
	}

	//Center index.
	_SHORT centerIndex = vertexIndex; vertexIndex ++;

	//Start index.
	_SHORT startIndex = vertexIndex;
	//For each segment, add a triangle to the sides triangle list.
	for(_INTEGER i = segments;i > 0;i --)
	{
		//Set three indices (1 triangle) per segment.
		*indexArray = centerIndex; indexArray ++;
		*indexArray = vertexIndex; indexArray ++;
		*indexArray = (i > 1) ? (vertexIndex + 1) : startIndex; indexArray ++; vertexIndex ++;
	}

	//Get vertex.
	_FLOAT3* vertex = CVertexAttribute::getFloat3Vertex();
#ifdef _DEBUG
	assert(vertex != _NULL);
	assert(getVertexCount() >= 11);
#endif

	//Get normal.
	_FLOAT3* normal = CNormalAttribute::getFloat3Normal();
#ifdef _DEBUG
	assert(normal != _NULL);
	assert(getNormalCount() >= 11);
	assert(getNormalCount() == getVertexCount());
#endif

	//Get coordinates.
	_FLOAT2* coordinates = CTextureAttribute::getFloat2Texture();
#ifdef _DEBUG
	assert(coordinates != _NULL);
	assert(getTextureCount() >= 11);
	assert(getTextureCount() == getVertexCount());
#endif

	//Compute delta.
	_FLOAT rDeltaSegAngle = 2.0f * (_FLOAT)D3DX_PI / (_FLOAT)segments;
	_FLOAT rSegmentLength = 1.0f / (_FLOAT)segments;
	_FLOAT ny0 = (90.0f - (_FLOAT)D3DXToDegree(atan(height / radius))) / 90.0f;

	//Position index.
	_INTEGER positionIndex = 0;
	//For each segment, add a triangle to the sides triangle list
	for(_INTEGER i = 0; i < segments;i ++)
	{
		_FLOAT x0 = radius * sinf(i * rDeltaSegAngle);
		_FLOAT z0 = radius * cosf(i * rDeltaSegAngle);

		vertex[positionIndex][0] = 0.0f;
		vertex[positionIndex][1] = 0.0f + height / 2.0f;
		vertex[positionIndex][2] = 0.0f;
		normal[positionIndex][0] = x0;
		normal[positionIndex][1] = ny0;
		normal[positionIndex][2] = z0;
		coordinates[positionIndex][0] = 1.0f - rSegmentLength * (_FLOAT)i;
		coordinates[positionIndex][1] = 0.0f;
		positionIndex ++;

		vertex[positionIndex][0] = x0;
		vertex[positionIndex][1] = 0.0f - height / 2.0f;
		vertex[positionIndex][2] = z0;
		normal[positionIndex][0] = x0;
		normal[positionIndex][1] = ny0;
		normal[positionIndex][2] = z0;
		coordinates[positionIndex][0] = 1.0f - rSegmentLength * (_FLOAT)i;
		coordinates[positionIndex][1] = 1.0f;
		positionIndex ++;
	}

	//Create the bottom triangle fan: Center vertex
	vertex[positionIndex][0] = 0.0f;
	vertex[positionIndex][1] = 0.0f - height / 2.0f;
	vertex[positionIndex][2] = 0.0f;
	normal[positionIndex][0] = 0.0f;
	normal[positionIndex][1] = -1.0f;
	normal[positionIndex][2] = 0.0f;
	coordinates[positionIndex][0] = 0.5f;
	coordinates[positionIndex][1] = 0.5f;
	positionIndex ++;

	//Create the bottom triangle fan: Edge vertices
	for(_INTEGER i = segments;i >= 0;i --)
	{
		_FLOAT x0 = radius * sinf(i * rDeltaSegAngle);
		_FLOAT z0 = radius * cosf(i * rDeltaSegAngle);
	
		vertex[positionIndex][0] = x0;
		vertex[positionIndex][1] = 0.0f - height / 2.0f;
		vertex[positionIndex][2] = z0;
		normal[positionIndex][0] = 0.0f;
		normal[positionIndex][1] = -1.0f;
		normal[positionIndex][2] = 0.0f;

		_FLOAT tu0 = (0.5f * sinf(i * rDeltaSegAngle)) + 0.5f;
		_FLOAT tv0 = (0.5f * cosf(i * rDeltaSegAngle)) + 0.5f;

		coordinates[positionIndex][0] = tu0;
		coordinates[positionIndex][1] = tv0;
		positionIndex ++;
	}
}
